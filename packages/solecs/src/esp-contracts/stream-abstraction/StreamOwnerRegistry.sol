...
contract StreamOwnerRegistry is IStreamOwnerRegistry {
    ...
    constructor(address _router, address _SMIC, address _TLSIC, address _SSIC) {
        router = _router;
        SMIC = _SMIC;
        TLSIC = _TLSIC;
        SSIC = _SSIC;
     ...
     */
    function addOrRemoveTopLevelSystem(uint256 _stream, address _system, bool _add) public onlyStreamOwner(_stream) {
    ...
    @param _add boolean for adding (true) or removing (false) a top level system from a stream
     */
    function addOrRemoveStreamSystem(uint256 _stream, address _system, bool _add) public onlyStreamOwner(_stream) {
        bytes memory boolUpdate = abi.encodePacked(_add);
