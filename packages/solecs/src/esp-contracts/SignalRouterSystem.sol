...
contract SignalRouterSystem is System, ISignalRouterSystem { 
    ...
    /**@param _streamCall preserves the integrity of the _stream ID for systems requesting state updates via reentrancy within a single call chain */
    uint256 public _streamCall;

    constructor(IWorld _world, uint256 _SMIC_ID, uint256 _TLSIC_ID, uint256 _SSIC_ID) System(_world, address(0)) {
        _this = address.this;
        _worldAddress = address(_world);
        // Deploys necessary registry & indeces linked to this router
        RouterSMIC = new StreamMemberIndexComponent(_this, _worldAddress, _SMIC_ID);
        RouterTLSIC = new TopLevelSystemIndexComponent(_this, _worldAddress, _TLSIC_ID);
        RouterSSIC = new StreamSystemIndexComponent(_this, _worldAddress, _SSIC_ID);
    ...
    function execute(bytes memory arguments) public override returns (bytes memory) {
        ...
        if (tx.origin == msg.sender) {
            ...
            require(RouterTLSIC.getValue(uint256(keccak256(abi.encode(_stream, _system)))), "System not valid at top level for stream");
            
            // Reentrancy check
            _streamCall = _stream;

            // Calls valid StreamSystem
            ISystem(_stream).execute(_arguments);
        } else {
            // executes if the caller is another contract
            (address _component, bytes memory keys, bytes memory value) = abi.decode(_arguments, (address, bytes, bytes));
